...
def main():
    ...
    p.add_argument('--timesteps',type=int,default=1000); p.add_argument('--num-basis',type=int,default=256); p.add_argument('--sigma',type=float,default=0.06); p.add_argument('--gaussian-channels',type=int,default=-1); p.add_argument('--fixed-basis',action='store_true')
    ...
    field=ContinuousGaussianField(args.num_basis,args.sigma,channels,gaussian_channels=gch,trainable_basis=not args.fixed_basis,device=args.device).to(args.device)
