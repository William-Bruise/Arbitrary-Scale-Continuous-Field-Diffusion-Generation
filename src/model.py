...
class LatentUNetDenoiser(nn.Module):
    def __init__(self, k: int, channels: int = 1, base: int = 64, tdim: int = 128):
        super().__init__()
        side = int(k ** 0.5)
        assert side * side == k
    ...
    def forward(self, x_t, t):
        ...
        temb = self.temb(t)
        x1 = self.down1(self.in_conv(x), temb)
        xm = self.mid2(self.mid1(self.downsample(x1), temb), temb)
        xu = self.upsample(xm)
        if xu.shape[-2:] != x1.shape[-2:]:
            xu = torch.nn.functional.interpolate(xu, size=x1.shape[-2:], mode="nearest")
        xu = self.up_reduce(self.up1(torch.cat([xu, x1], dim=1), temb))
